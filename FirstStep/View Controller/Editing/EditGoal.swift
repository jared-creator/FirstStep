...
struct EditGoal: View {
    ...
    var body: some View {
        ...
        VStack {
            ...
            GroupBox {
                VStack(alignment: .leading, spacing: 10) {
                    TextField("", text: $goal.name, prompt: Text("e.g. No Sugar")
                        .foregroundStyle(.secondary))
                    
                    DatePicker("Start Date:", selection: $goal.startDate)
                        .foregroundStyle(.secondary)
        ...
        .onAppear {
            ...
            color = Color(hex: goal.cardColor) ?? .red
            loadImage()
        }
        ...
        .task(id: color) {
            goal.cardColor = color.hexString()
        }
    }
    
    func loadImage() {
        guard let habitImage = goal.image else { return }
...
#Preview {
    do {
        ...
        let example = Goals(name: "No Popcorn", startDate: .now, cardColor: "#FD3A69")
        return EditGoal(goal: example)
